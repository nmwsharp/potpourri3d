...
class MeshHeatMethodDistanceSolver:
    def __init__(
        self, V: np.ndarray, F: np.ndarray, t_coef: float = 1.0, use_robust: bool = True
    ) -> None:
    ...
    def compute_distance(self, v_ind: np.ndarray) -> float:
        return self.bound_solver.compute_distance(v_ind)

    def compute_distance_multisource(self, v_inds: np.ndarray) -> float:
        return self.bound_solver.compute_distance_multisource(v_inds)


def compute_distance(V: np.ndarray, F: np.ndarray, v_ind: np.ndaaray) -> np.ndarray:
    solver = MeshHeatMethodDistanceSolver(V, F)
    return solver.compute_distance(v_ind)


def compute_distance_multisource(
    V: np.ndarray, F: np.ndarray, v_inds: np.ndarray
) -> np.ndarray:
    solver = MeshHeatMethodDistanceSolver(V, F)
...
class MeshVectorHeatSolver:
    def __init__(self, V: np.ndarray, F: np.ndarray, t_coef: float = 1.0):
        validate_mesh(V, F, force_triangular=True, test_indices=True)
        self.bound_solver = pp3db.MeshVectorHeatMethod(V, F, t_coef)

    def extend_scalar(self, v_inds: np.ndarray, values: np.ndarray) -> np.ndarray:
        if len(v_inds) != len(values):
            raise ValueError(
                "source vertex indices and values array should be same shape"
            )
        return self.bound_solver.extend_scalar(v_inds, values)

    def get_tangent_frames(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        return self.bound_solver.get_tangent_frames()

    def get_connection_laplacian(self) -> scipy.sparse.csr_matrix:
        return self.bound_solver.get_connection_laplacian()

    def transport_tangent_vector(self, v_ind: int, vector: np.ndarray) -> np.ndarray:
        if len(vector) != 2:
    ...
    def transport_tangent_vectors(
        self, v_inds: np.ndarray, vectors: np.ndarray
    ) -> np.ndarray:
        if len(v_inds) != len(vectors):
            raise ValueError(
                "source vertex indices and values array should be same length"
            )
        return self.bound_solver.transport_tangent_vectors(v_inds, vectors)

    def compute_log_map(self, v_ind: int) -> np.ndarray:
...
class EdgeFlipGeodesicSolver:
    ...
    def find_geodesic_path(self, v_start: int, v_end: int) -> np.ndarray:
        return self.bound_solver.find_geodesic_path(v_start, v_end)

    def find_geodesic_path_poly(self, v_list: List[int]) -> np.ndarray:
        return self.bound_solver.find_geodesic_path_poly(v_list)

    def find_geodesic_loop(self, v_list: List[int]) -> np.ndarray:
        return self.bound_solver.find_geodesic_loop(v_list)


def cotan_laplacian(
    V: np.ndarray, F: np.ndarray, denom_eps: float = 0.0
...
def face_areas(V: np.ndarray, F: np.ndarray) -> np.ndarray:
    """Computes the areas spanned by each triangle in a surface mesh.

    Args:
        V (np.ndarray): The vertex positions.
        F (np.ndarray): The face indices.

    Returns:
        np.ndarray: The face areas as a numpy array.
    """
    validate_mesh(V, F, force_triangular=True)
...
def vertex_areas(V: np.ndarray, F: np.ndarray) -> np.ndarray:
    """Compute the vertex areas for a given surface mesh.
    ...
    Returns:
        np.ndarray: The vertex areas as a numpy array.
    """
    validate_mesh(V, F, force_triangular=True)
    nV = V.shape[0]
